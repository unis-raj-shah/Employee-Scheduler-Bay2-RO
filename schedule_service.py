...
def get_orders_for_scheduling(target_date: Optional[datetime] = None):
    ...
    try:
        ...
        print(f"DEBUG: Target date: {target_date.strftime('%Y-%m-%d') if target_date else 'None'}")
        print(f"DEBUG: Forecast data: shipping={total_shipping_pallets}, incoming={total_incoming_pallets}, cases={cases_to_pick}, staged={staged_pallets}")
...
def assign_employees_to_roles(required_roles: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    Returns:
        Dictionary mapping flattened role keys to lists of assigned employees
    """
    assigned_employees = {}
...
def run_scheduler() -> Optional[Dict[str, Any]]:
    ...
    print(f"DEBUG: Fetching data for TOMORROW: {tomorrow_start.strftime('%Y-%m-%d')}")
    forecast_data_tomorrow, _ = get_orders_for_scheduling(tomorrow_start)
    
    # Get orders for day after using day after's date range
    print(f"DEBUG: Fetching data for DAY AFTER: {day_after_start.strftime('%Y-%m-%d')}")
    ...
    for operation, roles in required_roles_tomorrow.items():
        for role, required_count in roles.items():
            base_role = role.replace('_', ' ')  # Convert to space format to match assigned_employees keys
            assigned_count = len(assigned_employees_tomorrow.get(base_role, []))
            if assigned_count < required_count:
                shortage_key = f"{operation}_{role.replace(' ', '_')}"
                if shortage_key not in shortages:
                    shortages[shortage_key] = 0
                shortages[shortage_key] += required_count - assigned_count
    ...
    # Create flattened staff counts for email from the required roles
    flattened_tomorrow_staff = {}
    for operation, roles in required_roles_tomorrow.items():
        for role, count in roles.items():
            role_key = f"{operation}_{role.replace(' ', '_')}"
            flattened_tomorrow_staff[role_key] = count
    
    flattened_day_after_staff = {}
    for operation, roles in required_roles_day_after.items():
        for role, count in roles.items():
            role_key = f"{operation}_{role.replace(' ', '_')}"
            flattened_day_after_staff[role_key] = count
    
    send_combined_forecast_email(tomorrow_data, day_after_data, 
                               flattened_tomorrow_staff, flattened_day_after_staff, 
                               shortages)
