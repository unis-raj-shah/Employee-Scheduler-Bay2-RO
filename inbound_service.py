...
def get_incoming_data(target_date: Optional[Any] = None) -> Dict[str, float]:
    """
    Get inbound receipt data for forecasting.
    
    Args:
        target_date: Target date for fetching incoming data
    
    Returns:
        Dictionary with incoming pallet count
    """
    try:
        print(f"DEBUG: get_incoming_data called with target_date: {target_date.strftime('%Y-%m-%d') if target_date else 'None'}")
